...
def d_block(inp, fil, p=True):
    route2 = Conv2D(filters=fil, kernel_size=3, padding='same', kernel_initializer='he_normal',
                    bias_initializer='zeros')(inp)
    route2 = LeakyReLU(0.01)(route2)
    if p:
        route2 = AveragePooling2D()(route2)
    route2 = Conv2D(filters=fil, kernel_size=3, padding='same', kernel_initializer='he_normal',
                    bias_initializer='zeros')(route2)
...
class GAN(object):
    ...
    def AdModel(self):
        ...
        self.AM.compile(optimizer=Adam_lr_mult(self.LR, beta_1=0, beta_2=0.99, decay=0.00001,
                                               multipliers=learning_rate_multipliers), loss='mse')
    ...
    def MixModel(self):
        ...
        self.MM.compile(optimizer=Adam_lr_mult(self.LR, beta_1=0, beta_2=0.99, decay=0.00001,
                                               multipliers=learning_rate_multipliers), loss='mse')
    ...
    def MixModelD(self):
        ...
        self.DMM.compile(optimizer=Adam(self.LR, beta_1=0, beta_2=0.99, decay=0.00001),
                         loss=['mse', 'mse', partial_gp_loss])
...
class WGAN(object):
    ...
    def train_dis(self):
        ...
        train_data = [self.im, noise(BATCH_SIZE), noiseImage(BATCH_SIZE), self.ones]
    ...
    def train_mix_d(self):
        ...
        # Train
        d_loss = self.MixModelD.train_on_batch([images] + n + [noiseImage(BATCH_SIZE), self.ones],
                                               [self.ones, self.nones, self.ones])
